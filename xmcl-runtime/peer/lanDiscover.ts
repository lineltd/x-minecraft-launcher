...
function setup(discover: MinecraftLanDiscover, lanScope: Set<string>, allPeers: Record<string, PeerSession>, logger: Logger) {
  discover.bind().then(() => {
    logger.log('Minecraft LAN discover ready')
  }, (e) => {
    logger.error(new Error('Fail to bind Minecraft LAN discover', { cause: e }))
  })

  discover.socket.on('error', (e) => {
    logger.error(new Error('Minecraft discover socket error', { cause: e }))
  })

  discover.on('discover', (info) => {
    const peers = Object.values(allPeers).filter(c => c.connection.state() === 'connected')
...
export function createLanDiscover(peers: Record<string, PeerSession>, logger: Logger) {
  ...
  setup(discover, set, peers, logger)
  setup(discoverV6, set, peers, logger)
