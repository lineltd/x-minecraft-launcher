import { ConnectionUserInfo, RTCSessionDescription, TransferDescription } from '@xmcl/runtime-api'
...
export class PeerSession {
  ...
  constructor(
    ...
    this.connection.addEventListener('datachannel', (e) => {
      ...
      if (channel.protocol === 'minecraft') {
        ...
        if (!this.context.isFileShared(fileName)) {
          // reject the file
          ...
          this.context.createSharedFileReadStream(filePath).on('data', (data: Buffer) => {
            channel.send(data)
          })
  ...
  private setChannel(channel: RTCDataChannel) {
    channel.addEventListener('message', async (e) => {
      ...
      this.context.handlers[message.type as string]?.call(this, message.payload)
    })
    channel.addEventListener('close', () => {
      this.close()
    })
    channel.addEventListener('open', () => {
      ipcRenderer.invoke('get-user-info').then((info) => {
        this.send(MessageIdentity, { remoteId: this.context.id, ...info })
      })
      if (!this.remoteId) {
